...
export default function FetchSpecificHeroSection() {
  const [section, setSection] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const { toast } = useToast();
  
  // The specific hero section ID you want to fetch
  const HERO_SECTION_ID = "682e0d584b70727b50df6fcd";
  ...
  useEffect(() => {
    const fetchHeroSection = async () => {
      try {
        ...
        const response = await fetch(`/api/admin/hero-sections/${HERO_SECTION_ID}`);
        ...
        if (data.success && data.section) {
          ...
          toast({
            title: "Error",
            description: data.message || "Failed to fetch hero section",
            variant: "destructive",
          });
        }
      } catch (error: any) {
